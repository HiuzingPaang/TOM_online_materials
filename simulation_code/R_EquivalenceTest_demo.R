...
for (irun in c(1,2)){
  ...
  for (ifile in seq(1,9801)){
    ...
    m_d = mean(delta)
    s_d = sd(delta)
    obj = TOSTone(m=m_d,mu=0,sd=s_d,n=nsub,low_eqbound_d=-0.21,high_eqbound_d=0.21,alpha=0.05)
    tmp[ifile] = obj$TOST_t2
    dev.off()
    cat('\014')
  }
  write.table(tmp,file=paste('.\\R_run_t_',as.character(irun),'_',as.character(nsub),'.txt',sep=''),sep=' ',row.names=FALSE,col.names=FALSE)
}
